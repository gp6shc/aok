...
class QuestionRequest extends PostRequest
{
    ...
    public function SearchAdvanced($search_text=null)
    {
    	// the /search method does not have the '/questions' prefix
    	// so we need to reset the category
    	$this->url->SetCategory('search/advanced');
    
    	if($search_text !== null)
    		$this->InQuery($search_text);
    ...
    /**
     * \param $search_text the query to match anywhere
     * \return the current instance
     */
    public function InQuery($search_text)
    {
    	$this->url->SetQueryStringParameter('q', $search_text);
    	return $this;
    }
