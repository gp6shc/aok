...
class Capiton_Twitter_Widget extends WP_Widget {
	var $prefix;
	var $textdomain;
	
	
	function Capiton_Twitter_Widget() {
		$this->prefix = '';
		$this->textdomain = 'twitter-widget';
		
		$this->plugin_file = 'twitter-widget/twitter-widget.php';
		...
		parent::WP_Widget("{$this->prefix}-twitter-widget", __('Twitter', $this->textdomain), $widget_ops);
	...
	function plugin_action_links( $actions, $plugin_file ) {
			if ( $plugin_file == $this->plugin_file && $this->settings_url)
				$actions[] = '<a href="'.$this->settings_url.'">' . __('Settings', 'core') .'</a>';
	...
	function plugin_row_meta( $plugin_meta, $plugin_file ){
			if ( $plugin_file == $this->plugin_file ) {
				//$plugin_meta[] = '<a href="'.$this->donate_url.'">' . __('Donate', 'core') .'</a>';
				//$plugin_meta[] = '<a href="'.$this->support_url.'">' . __('Support', 'core') .'</a>';
			}
...
add_action('widgets_init', 'register_capiton_twitter_widget');

function register_capiton_twitter_widget() {
	register_widget('Capiton_Twitter_Widget');
}