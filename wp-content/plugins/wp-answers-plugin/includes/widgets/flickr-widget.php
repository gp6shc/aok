...
class Capiton_Flickr_Widget extends WP_Widget {
	 ...
	 * @since 0.7
	 */
	function Capiton_Flickr_Widget() {
		$this->prefix = '';
		$this->textdomain = 'flickr-widget';
		...
		$this->WP_Widget("{$this->prefix}-flickr-widget", __('Flickr', $this->textdomain), $widget_ops); 
	...
	function form($instance) {  
		...
		$size = array( 's' => __( 'Small', $this->textdomain ), 'm' => __( 'Medium', $this->textdomain ) );
...
add_action('widgets_init', 'register_capiton_flickr_widget');

function register_capiton_flickr_widget() {
	register_widget('Capiton_Flickr_Widget');
}